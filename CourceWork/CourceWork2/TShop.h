...
private:
	...
	std::map<int, std::shared_ptr<sTime>> Orders;		//Ассоциативный массив структур
...
public:
	...
	TShop(const TShop& T)
	{
		std::copy(T.Orders.begin(), next(T.Orders.begin(), T.quantity), inserter(Orders, Orders.begin()));
		quantity = T.quantity;
	}
